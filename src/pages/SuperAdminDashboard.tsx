...
import {
  Users, Clock, FolderKanban, IndianRupee, CheckCircle2,
  TrendingUp, Film, Code2, ArrowUpRight, Calendar, ArrowRight,
} from 'lucide-react';
import { employees, projects, tasks, attendanceRecords, transactions, departments } from '@/data/mockData';
...
export function SuperAdminDashboard(props: SuperAdminDashboardProps) {
  ...
  return (
    <div className="min-w-0 space-y-6" aria-label={`${props.user.role} dashboard`}>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <SummaryCard title="Attendance" icon={Clock} tone="bg-[#FFF1F2] text-[#ED0016]" value={`${presentToday}/${totalEmp}`} to="/attendance">
          <div className="mt-3 flex flex-wrap gap-4 text-xs">
            <span className="flex items-center gap-1 text-green-600"><span className="size-2 rounded-full bg-green-500" />Present: {presentToday}</span><span className="flex items-center gap-1 text-amber-500"><span className="size-2 rounded-full bg-amber-400" />On Leave: {onLeaveToday}</span>
          </div>
        ...
        <SummaryCard title="Tasks" icon={CheckCircle2} tone="bg-purple-50 text-purple-600" value={<>{totalTasks} <span className="text-sm font-normal text-[#9CA3AF]">total</span></>} to="/tasks">
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
            <span className="flex items-center gap-1 text-green-600"><span className="size-2 shrink-0 rounded-full bg-green-500" />Done: {completedTasks}</span>
            <span className="flex items-center gap-1 text-amber-500"><span className="size-2 shrink-0 rounded-full bg-amber-400" />Pending: {pendingTasks}</span>
            <span className="flex items-center gap-1 text-amber-500"><span className="size-2 shrink-0 rounded-full bg-amber-400" />Overdue: {overdueTasks}</span>
          </div>
            ...
            <tbody className="divide-y divide-[#F3F4F6]">
              {activeProjects.map(p => (
                <tr key={p.id} onClick={() => navigate(`/projects/${p.id}`)} className="cursor-pointer transition-colors hover:bg-[#F9FAFB]">
                  <td className="px-4 py-3.5"><button type="button" onClick={event => { event.stopPropagation(); navigate(`/projects/${p.id}`); }} className="text-left text-sm font-semibold! text-slate-600 hover:underline">{p.name}</button><p className="text-xs text-[#9CA3AF]">{p.code}</p></td>
